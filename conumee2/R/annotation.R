...
CNV.create_anno <- function(
  ...
  object@args <- as.list(sapply(
    unique(names(c(a1, a2))),
  ...
  if (all(is.element(array_type, c("450k", "EPIC", "EPICv2")))) {
    # human annotations
    if (chrXY) {
      object@genome <- data.frame(
    ...
    tbl.chromInfo <- tbl$chromInfo[
      match(object@genome$chr, tbl$chromInfo$chrom),
      "size"
    ]
    object@genome$size <- tbl.chromInfo
    tbl.gap <- tbl$gap[is.element(tbl$gap$chrom, object@genome$chr), ]
    object@gap <- sort(GRanges(
      as.vector(tbl.gap$chrom),
      IRanges(tbl.gap$chromStart + 1, tbl.gap$chromEnd),
      seqinfo = Seqinfo(object@genome$chr, object@genome$size)
    ))

    if (genome == "hg19") {
      tbl.cytoBand <- tbl$cytoBand[
        is.element(tbl$cytoBand$chrom, object@genome$chr),
      ]
      ...
      pq <- sapply(
        split(
          tbl.cytoBand$chromEnd[grepl("p", tbl.cytoBand$name)],
          as.vector(tbl.cytoBand$chrom[grepl("p", tbl.cytoBand$name)])
        ),
        max
      )
      object@genome$pq <- start(resize(
        subsetByOverlaps(object@gap, GRanges(names(pq), IRanges(pq, pq))),
        1,
        fix = "center"
      ))
    } else if (genome == "hg38") {
      cent.min <- sapply(
        split(
          tbl$centromeres$chromStart[which(is.element(
            tbl$centromeres$chrom,
            object@genome$chr
          ))],
          tbl$centromeres$chrom[which(is.element(
            tbl$centromeres$chrom,
            object@genome$chr
          ))]
        ),
      ...
      object@genome$pq <- start(sort(resize(
        GRanges(
          as.vector(names(cent.min)),
          IRanges(start = cent.min, stop = cent.max),
          seqinfo = Seqinfo(object@genome$chr, object@genome$size)
        ),
        1,
        fix = "center"
    ...
    if (all(array_type == "450k")) {
      ...
      probes <- probesEPICv2[
        probesEPICv2$Methyl450_Loci != "" & probesEPICv2$EPICv1_Loci != ""
      ]
      probes$EPICv1_Loci <- probes$Methyl450_Loci <- NULL
    }
    ...
    if (!is.null(detail_regions)) {
      ...
      if (class(detail_regions) == "GRanges") {
        ...
        if (any(grepl("name", names(values(detail_regions))))) {
          values(object@detail)$name <- values(detail_regions)[[grep(
            "name",
            names(values(detail_regions))
          )[1]]]
        }
        if (any(grepl("IRanges", sapply(values(detail_regions), class)))) {
          values(object@detail)$thick <- values(detail_regions)[[grep(
            "IRanges",
            sapply(values(detail_regions), class)
          )[1]]]
        }
    ...
    if (!is.element("thick", names(values(object@detail)))) {
      values(object@detail)$thick <- resize(
        ranges(object@detail),
        fix = "center",
        1e+06
      )
    }
    ...
    anno.tile <- CNV.create_bins(
      genome.anno = object@genome,
      bin_minsize = bin_minsize,
      genome.gap = object@gap,
      genome.exclude = object@exclude
    )
    message(" - ", length(anno.tile), " bins created")

    message("merging bins")
    object@bins <- CNV.merge_bins(
      genome.anno = object@genome,
      genome.tile = anno.tile,
      bin_minprobes = bin_minprobes,
      genome.probes = object@probes,
    ...
    bin_genes <- sapply(
      lapply(
        sapply(
          split(
            object@probes$genes[queryHits(o)],
            names(object@bins)[subjectHits(o)]
          ),
          function(x) na.omit(unlist(strsplit(x, split = ";")))
        ),
        unique
      ),
      paste,
    ...
    return(object)
  } else if (array_type == "mouse") {
    # mouse annotations

    if (chrXY) {
      object@genome <- data.frame(
        chr = paste("chr", c(1:19, "X", "Y"), sep = ""),
        stringsAsFactors = FALSE
      )
    } else {
      object@genome <- data.frame(
    ...
    tbl.gap <- mouse_annotation[[2]][
      is.element(mouse_annotation[[2]]$chrom, object@genome$chr),
    ]
    object@gap <- sort(GRanges(
      as.vector(tbl.gap$chrom),
      IRanges(tbl.gap$chromStart + 1, tbl.gap$chromEnd),
    ...
    ind.chr <- is.element(
      mouse_annotation[[3]]$CHR,
      unlist(lapply(strsplit(object@genome$chr, "r"), function(x) x[2]))
    )

    mouse_probes <- GRanges(
      as.vector(paste("chr", mouse_annotation[[3]]$CHR[ind.chr], sep = "")),
      IRanges(
        start = mouse_annotation[[3]]$MAPINFO[ind.chr],
        end = mouse_annotation[[3]]$MAPINFO[ind.chr]
      ),
      seqinfo = Seqinfo(object@genome$chr, object@genome$size, genome = "mm10")
    )
    ...
    mouse_probes <- sort(mouse_probes[
      substr(names(mouse_probes), 1, 2) == "cg"
    ])
...
CNV.create_bins <- function(
  genome.anno,
  bin_minsize = 50000,
  genome.gap,
  genome.exclude
) {
  genome.tile <- sort(tileGenome(
    Seqinfo(genome.anno$chr, genome.anno$size),
    tilewidth = bin_minsize,
    cut.last.tile.in.chrom = TRUE
  ))
  # setdiff for gaps (on every second window to avoid merging)
  genome.tile <- sort(c(
    setdiff(genome.tile[seq(1, length(genome.tile), 2)], genome.gap),
    setdiff(genome.tile[seq(2, length(genome.tile), 2)], genome.gap)
  ))
  # setdiff for exluded regions
  genome.tile <- sort(c(
    setdiff(genome.tile[seq(1, length(genome.tile), 2)], genome.exclude),
    setdiff(genome.tile[seq(2, length(genome.tile), 2)], genome.exclude)
  ))
  return(genome.tile)
}
...
CNV.merge_bins <- function(
  ...
  genome.tile.df <- as.data.frame(genome.tile)[, c(
    "seqnames",
    "start",
    "end",
    "probes"
  ...
  genome.tile.df.bin <- do.call(
    rbind,
    lapply(split(genome.tile.df, genome.tile.df$seqnames), function(mdf) {
      while (min(mdf$probes) < bin_minprobes) {
        ...
        if (is.na(mwn)) {
          if (verbose)
            message(
              paste(mdf[mw, 1:3], collapse = "-"),
              " has only ",
              mdf[mw, "probes"],
              " probes and cannot be merged - remove"
            )
          mdf <- mdf[-mw, ]
        } else {
          # merge
  ...
  genome.tile.bin <- sort(GRanges(
    genome.tile.df.bin$seqnames,
    IRanges(genome.tile.df.bin$start, genome.tile.df.bin$end),
    seqinfo = seqinfo(genome.tile)
  ))
  genome.tile.bin <- genome.tile.bin[width(genome.tile.bin) <= bin_maxsize]

  values(genome.tile.bin)$probes <- countOverlaps(
    genome.tile.bin,
    genome.probes
  )
  values(genome.tile.bin)$midpoint <- as.integer(
    start(genome.tile.bin) +
      (end(genome.tile.bin) - start(genome.tile.bin)) / 2
  )

  names(genome.tile.bin) <- paste(
    as.vector(seqnames(genome.tile.bin)),
    formatC(
      unlist(lapply(table(seqnames(genome.tile.bin)), seq)),
