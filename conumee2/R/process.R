...
setMethod(
  "CNV.fit",
  signature(query = "CNV.data", ref = "CNV.data", anno = "CNV.anno"),
  function(query, ref, anno, intercept = TRUE) {
    if (ncol(query@intensity) == 0) stop("query intensities unavailable, run CNV.load")
    if (ncol(ref@intensity) == 0) stop("reference set intensities unavailable, run CNV.load")

    if (ncol(query@intensity) != 1) message("using multiple query samples")
    if (ncol(ref@intensity) == 1)
      warning(
        "reference set contains only a single sample. use more samples for better results."
      )
    ...
    if (!all(is.element(p, rownames(query@intensity)))) {
      ...
      if ("IlmnID" %in% colnames(values(anno@probes))) {
        # if not, check if full IlmnID identifiers are used
        if (all(is.element(unique(anno@probes$IlmnID), rownames(query@intensity)))) {
          # if yes, change the data object
          query@intensity <- as.data.frame(query@intensity[
            anno@probes$IlmnID,
          ]) # If we only have one sample this becomes a list and gets sad
    ...
    for (i in 1:ncol(query@intensity)) {
      message(
        paste(colnames(query@intensity)[i]),
        " (",
        round(i / ncol(query@intensity) * 100, digits = 3),
        "%",
        ")",
        sep = ""
      )
      ...
      if (any(!r))
        message(
          "query sample seems to also be in the reference set. not used for fit."
        )
      if (intercept) {
        ref.fit <- lm(
          y ~ .,
          data = data.frame(
            y = log2(query@intensity[p, i]),
            X = log2(ref@intensity[p, r])
          )
        )
      } else {
        ref.fit <- lm(
      ...
      object@fit$coef <- cbind(
        object@fit$coef,
        as.numeric(ref.fit$coefficients[-1])
      )

      ref.predict <- predict(ref.fit)
      ref.predict[ref.predict < 0] <- 0

      object@fit$ratio <- cbind(
        object@fit$ratio,
        log2(query@intensity[p, i]) - ref.predict[p]
      )
    }
    ...
    for (i in 1:ncol(query@intensity)) {
      object@fit$noise <- c(
        object@fit$noise,
        sqrt(mean(
          (object@fit$ratio[-1, i] -
            object@fit$ratio[-nrow(object@fit$ratio), i])^2,
          na.rm = TRUE
        ))
      )
    }
...
setMethod("CNV.bin", signature(object = "CNV.analysis"), function(object) {
  ...
  o1 <- as.matrix(findOverlaps(
    query = object@anno@bins,
    subject = object@anno@probes
  ))
  o2 <- data.frame(
    bin = names(object@anno@bins)[o1[, "queryHits"]],
    probe = names(object@anno@probes)[o1[, "subjectHits"]],
  ...
  for (i in 1:ncol(object@fit$ratio)) {
    message(
      paste(colnames(object@fit$ratio)[i]),
      " (",
      round(i / ncol(object@fit$ratio) * 100, digits = 3),
      "%",
      ")",
      sep = ""
    )

    object@bin$ratio[[i]] <- sapply(
      split(object@fit$ratio[o2[, "probe"], i], o2[, "bin"]),
      median,
      na.rm = TRUE
    )[names(object@anno@bins)]

    object@bin$variance[[i]] <- sapply(
      split(object@fit$ratio[o2[, "probe"], i], o2[, "bin"]),
      var,
      na.rm = TRUE
    )[names(object@anno@bins)]
    ...
    object@bin$shift <- c(
      ...
      optim(
        0,
        function(s) median(abs(object@bin$ratio[[i]] - s), na.rm = TRUE),
        method = "Brent",
        lower = -100,
        upper = 100
      )$par
...
setMethod("CNV.detail", signature(object = "CNV.analysis"), function(object) {
  ...
  if (length(object@anno@detail) == 0) {
    ...
    d1 <- as.matrix(findOverlaps(
      query = object@anno@detail,
      subject = object@anno@probes
    ))
    d2 <- data.frame(
      detail = values(object@anno@detail)$name[d1[, "queryHits"]],
      probe = names(object@anno@probes[d1[, "subjectHits"]]),
      stringsAsFactors = FALSE
    )

    object@detail$ratio <- vector(
      mode = "list",
      length = ncol(object@fit$ratio)
    )
    for (i in 1:ncol(object@fit$ratio)) {
      object@detail$ratio[[i]] <- sapply(
        split(object@fit$ratio[d2[, "probe"], i], d2[, "detail"]),
        median,
        na.rm = TRUE
...
setMethod(
  ...
  function(
    ...
    object@seg$args <- as.list(sapply(
      setdiff(unique(names(c(a1, a2))), c("object", "verbose")),
      function(an) if (is.element(an, names(a2))) a2[[an]] else a1[[an]],
      simplify = FALSE
    ))
    ...
    for (i in 1:ncol(object@fit$ratio)) {
      ...
      x1 <- DNAcopy::CNA(
        genomdat = object@bin$ratio[[i]][names(object@anno@bins)],
        chrom = as.vector(seqnames(object@anno@bins)),
        maploc = values(object@anno@bins)$midpoint,
        data.type = "logratio",
        sampleid = "sampleid"
      )
      ...
      x2 <- DNAcopy::segment(
        x = x1,
        weights = 1 / object@bin$variance[[i]][names(object@anno@bins)],
        verbose = verbose,
        min.width = min.width,
        nperm = nperm,
        alpha = alpha,
...
setMethod(
  ...
  function(
    ...
    for (i in 1:ncol(object@fit$ratio)) {
      ...
      segs <- GRanges(
        seqnames = segs$chrom,
        IRanges(start = segs$loc.start, end = segs$loc.end),
        seqinfo = Seqinfo(genome = object@anno@args$genome)
      )
      ...
      boots <- bootRanges(
        ...
        R = R,
        seg = seg,
        exclude = object@anno@exclude,
        proportionLength = proportionLength
      )
      boots$state <- factor(boots$state, levels = c(1, 2, 3))

      boots.state <- split(boots$log2, list(boots$iter, boots$state), sep = " ")
      boots.state.mean <- mean(sapply(
        boots.state[grep(" 1", names(boots.state))],
        mean
      )) #derive mean and sd estimation from bootstrap
      boots.state.mean <- c(
        boots.state.mean,
        mean(sapply(boots.state[grep(" 2", names(boots.state))], mean))
      )
      boots.state.mean <- c(
        boots.state.mean,
        mean(sapply(boots.state[grep(" 3", names(boots.state))], mean))
      )

      boots.state.sd <- mean(sapply(
        boots.state[grep(" 1", names(boots.state))],
        sd
      ))
      boots.state.sd <- c(
        boots.state.sd,
        mean(sapply(boots.state[grep(" 2", names(boots.state))], sd))
      )
      boots.state.sd <- c(
        boots.state.sd,
        mean(sapply(boots.state[grep(" 3", names(boots.state))], sd))
      )

      boots.state.low <- mapply(
        qnorm,
        boots.state.mean,
        boots.state.sd,
        p = (1 - conf) / 2
      ) # assume normal distr, two-sided
      boots.state.high <- mapply(
        qnorm,
        boots.state.mean,
        boots.state.sd,
        p = 1 - (1 - conf) / 2
      ...
      bins.amp <- sort(
        bins.total[bins.total >= boots.state.high[bins$state]],
        decreasing = TRUE
      )
      ...
      d2 <- data.frame(
        gene = values(detail.genes)$name[d1[, "queryHits"]],
        probe = names(object@anno@probes[d1[, "subjectHits"]]),
        stringsAsFactors = FALSE
      )
      detail.genes.ratio <- sapply(
        split(object@fit$ratio[d2[, "probe"], i], d2[, "gene"]),
        median,
        na.rm = TRUE
      )[values(detail.genes)$name]
      detail.genes.ratio <- detail.genes.ratio - object@bin$shift[i]
      detail.genes.seg <- round(sapply(
        split(
          seg$state[d1seg[, "subjectHits"]],
          values(detail.genes)$name[d1seg[, "queryHits"]]
        ),
        mean
      ))[values(detail.genes)$name]

      detail.regions.amp[[i]] <- sort(
        detail.genes.ratio[which(
          detail.genes.ratio >= boots.state.high[detail.genes.seg]
        )],
        decreasing = TRUE
      )
      detail.regions.del[[i]] <- sort(detail.genes.ratio[which(
        detail.genes.ratio <= boots.state.low[detail.genes.seg]
      )])
      ...
      if (sig_cgenes) {
        ...
        cancer.genes <- cancer_genes[setdiff(
          cancer_genes$SYMBOL,
          object@anno@detail$name
        )]
        d1 <- as.matrix(findOverlaps(
          query = cancer.genes,
          subject = object@anno@probes
        ...
        d2 <- data.frame(
          gene = values(cancer.genes)$SYMBOL[d1[, "queryHits"]],
          probe = names(object@anno@probes[d1[, "subjectHits"]]),
          stringsAsFactors = FALSE
        )
        cancer.genes.ratio <- sapply(
          split(object@fit$ratio[d2[, "probe"], i], d2[, "gene"]),
          median,
          na.rm = TRUE
        )[values(cancer.genes)$SYMBOL]
        cancer.genes.ratio <- cancer.genes.ratio - object@bin$shift[i]
        cancer.genes.seg <- round(sapply(
          split(
            seg$state[d1seg[, "subjectHits"]],
            values(cancer.genes)$SYMBOL[d1seg[, "queryHits"]]
          ),
          mean,
          na.rm = TRUE
        ))[values(cancer.genes)$SYMBOL]

        cancer.genes.amp[[i]] <- sort(
          cancer.genes.ratio[which(
            cancer.genes.ratio >= boots.state.high[cancer.genes.seg]
          )],
          decreasing = TRUE
        )
        cancer.genes.del[[i]] <- sort(cancer.genes.ratio[which(
          cancer.genes.ratio <= boots.state.low[cancer.genes.seg]
        )])
      }
    }
...
setMethod(
  "CNV.combine",
  signature(query.1 = "CNV.analysis", query.2 = "CNV.analysis"),
