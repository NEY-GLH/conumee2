...
#' x <- CNV.segment(CNV.detail(CNV.bin(CNV.fit(
#'   query = d["GroupB_1"],
...
#' @author Volker Hovestadt, Bjarne Daenekas \email{conumee@@hovestadt.bio}
#' @export
setGeneric("CNV.genomeggplot", function(object, ...) {
...
setMethod(
  ...
  function(
    ...
    for (i in 1:ncol(object@fit$ratio)) {
      ...
      plot <- plot +
        geom_vline(
          xintercept = .cumsum0(object@anno@genome[chr, "size"], right = TRUE),
          color = "grey"
        )
      ...
      if (centromere) {
        ...
        plot <- plot +
          geom_vline(
            xintercept = .cumsum0(object@anno@genome[chr, "size"]) +
      ...
      # detail
      if (ncol(object@anno@genome) != 2 & detail & length(object@detail) > 0) {
        # human array
...
setMethod(
  ...
  function(
    ...
    abline(
      v = .cumsum0(object@anno@genome$size, right = TRUE),
      col = "grey"
    )
    abline(
