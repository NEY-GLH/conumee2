  ...
  function(
    ...
    for (i in 1:ncol(object@fit$ratio)) {
      ...
      plot(
        ...
        xlim = c(
          0,
          sum(as.numeric(object@anno@genome[chr, "size"])) -
            0
        ),
        ylim = ylim,
        xaxs = "i",
      ...
      abline(
        v = .cumsum0(object@anno@genome[chr, "size"], right = TRUE),
        col = "grey"
      )
      if (centromere) {
        abline(
          v = .cumsum0(object@anno@genome[chr, "size"]) +
            object@anno@genome[chr, "pq"],
          col = "grey",
          lty = 2
        )
      }
      ...
      axis(
        1,
        at = .cumsum0(object@anno@genome[chr, "size"]) +
          object@anno@genome[chr, "size"] / 2,
      ...
      if (bins_cex == "standardized") {
        ...
        p_size <- round(
          0.7 * ((p_size - min(p_size)) / (max(p_size) - min(p_size))) + 0.2,
          digits = 2
        ) #scaling from 0.1:0.8 for cex using predefined bins to enable comparability between plots
      } else {
        p_size <- bins_cex
      }
      ...
      lines(
        chr.cumsum0[as.vector(seqnames(object@anno@bins))] +
      ...
      for (l in seq(length(object@seg$summary[[i]]$seg.median))) {
        lines(
          ...
          rep(
            min(ylim[2], max(ylim[1], object@seg$summary[[i]]$seg.median[l])),
            2
          ) -
            object@bin$shift[i],
          col = "darkblue",
      ...
      if (detail & length(object@detail) > 0 & ncol(object@anno@genome) == 2) {
        ...
        text(
          start(object@anno@detail) +
            (end(object@anno@detail) - start(object@anno@detail)) / 2 +
            chr.cumsum0[as.vector(seqnames(object@anno@detail))],
          ifelse(detail.ratio.above, detail.ratio, NA),
          labels = paste("  ", values(object@anno@detail)$name, sep = ""),
          adj = c(0, 0.5),
          srt = 90,
          col = "black"
        )
        text(
          start(object@anno@detail) +
            (end(object@anno@detail) - start(object@anno@detail)) / 2 +
            chr.cumsum0[as.vector(seqnames(object@anno@detail))],
          ifelse(detail.ratio.above, NA, detail.ratio),
          labels = paste(values(object@anno@detail)$name, "  ", sep = ""),
          adj = c(1, 0.5),
          srt = 90,
          col = "black"
        ...
        ncol(object@anno@genome) != 2 & detail & length(object@detail) > 0
      ) {
        #human array
        ...
        if (
          !length(object@detail$amp.detail.regions[[i]]) == 0 ||
            !length(object@detail$del.detail.regions[[i]]) == 0
        ) {
          #CNV focal was used

          sig.detail.regions.ratio <- c(
            object@detail$amp.detail.regions[[i]],
            object@detail$del.detail.regions[[i]]
          )

          sig.detail.regions.ratio[sig.detail.regions.ratio < ylim[1]] <- ylim[
            1
          ]
          sig.detail.regions.ratio[sig.detail.regions.ratio > ylim[2]] <- ylim[
            2
          ]
          sig.detail.regions.ratio.above <- (sig.detail.regions.ratio > 0 &
            sig.detail.regions.ratio < 0.85) |
            sig.detail.regions.ratio < -0.85

          sig.detail.regions <- object@anno@detail[
            object@anno@detail$name %in% names(sig.detail.regions.ratio)
          ]
          names(sig.detail.regions) <- sig.detail.regions$name
          sig.detail.regions.ratio <- sig.detail.regions.ratio[names(
            sig.detail.regions
          )]
          ...
          text(
            ...
            labels = paste("  ", names(sig.detail.regions), sep = ""),
            adj = c(0, 0.5),
            srt = 90,
            col = "red"
          )
          ...
          if (sig_cgenes) {
            ...
            sig.cancer.genes.sorted <- names(sort(
              c(
                object@detail$amp.cancer.genes[[i]],
                abs(object@detail$del.cancer.genes[[i]])
              ),
              decreasing = TRUE
            ))
            ...
            sig.cancer.genes.ratio <- c(
              object@detail$amp.cancer.genes[[i]],
              object@detail$del.cancer.genes[[i]]
            )[sig.cancer.genes.sorted[1:nsig_cgenes]]

            sig.cancer.genes.ratio[sig.cancer.genes.ratio < ylim[1]] <- ylim[1]
            sig.cancer.genes.ratio[sig.cancer.genes.ratio > ylim[2]] <- ylim[2]
            sig.cancer.genes.ratio.above <- (sig.cancer.genes.ratio > 0 &
            ...
            text(
              start(sig.cancer.genes) +
                (end(sig.cancer.genes) - start(sig.cancer.genes)) / 2 +
                chr.cumsum0[as.vector(seqnames(sig.cancer.genes))],
              ifelse(sig.cancer.genes.ratio.above, sig.cancer.genes.ratio, NA),
              labels = paste("  ", names(sig.cancer.genes), sep = ""),
              adj = c(0, 0.5),
              srt = 90,
              col = "red"
            )
            text(
              start(sig.cancer.genes) +
                (end(sig.cancer.genes) - start(sig.cancer.genes)) / 2 +
                chr.cumsum0[as.vector(seqnames(sig.cancer.genes))],
              ifelse(sig.cancer.genes.ratio.above, NA, sig.cancer.genes.ratio),
              labels = paste(names(sig.cancer.genes), "  ", sep = ""),
              adj = c(1, 0.5),
              srt = 90,
              col = "red"
...
setMethod(
  ...
  function(
    ...
    for (i in columns) {
      ...
      detail.gene <- object@anno@detail[match(
        name,
        values(object@anno@detail)$name
      )]
      ...
      plot(
        NA,
        xlim = c(start(detail.region), end(detail.region)),
        ylim = ylim,
        xaxt = "n",
        yaxt = "n",
        xlab = NA,
      ...
      axis(
        1,
        at = mean(c(start(detail.region), end(detail.region))),
        labels = as.vector(seqnames(detail.region)),
        tick = 0,
        las = 1
      )
      axis(
        1,
        at = start(detail.region),
        labels = format(
          start(detail.region),
          big.mark = ",",
          scientific = FALSE
      ...
      axis(
        1,
        at = end(detail.region),
        labels = format(end(detail.region), big.mark = ",", scientific = FALSE),
        las = 2,
        padj = 0
      )
      if (yaxt != "n")
      ...
      detail.bins <- names(object@bin$ratio[[i]])[as.matrix(findOverlaps(
        detail.region,
        object@anno@bins,
        maxgap = width(detail.region)
      ))[, 2]]
      detail.probes <- names(object@anno@probes)[as.matrix(findOverlaps(
        detail.region,
        object@anno@probes,
        maxgap = width(detail.region)
      ))[, 2]]
      ...
      detail.ratio[detail.ratio > ylim[2]] <- ylim[2]
      detail.ratio[detail.ratio < ylim[1]] <- ylim[1]
      detail.ratio.cols <- apply(
        colorRamp(cols)(
          (detail.ratio + max(abs(ylim))) /
            (2 *
              max(abs(ylim)))
        ),
        1,
      ...
      lines(
        start(object@anno@probes[detail.probes]),
        detail.ratio,
        type = "p",
        pch = 4,
        cex = 0.75,
        col = detail.ratio.cols
      )

      anno.bins.detail <- object@anno@bins[detail.bins]
      anno.bins.ratio <- object@bin$ratio[[i]][detail.bins] -
        object@bin$shift[i]
      anno.bins.ratio[anno.bins.ratio > ylim[2]] <- ylim[2]
      anno.bins.ratio[anno.bins.ratio < ylim[1]] <- ylim[1]
      lines(
        as.vector(rbind(
          rep(start(anno.bins.detail), each = 2),
          rep(end(anno.bins.detail), each = 2)
        )),
        as.vector(rbind(NA, anno.bins.ratio, anno.bins.ratio, NA)),
        col = "black",
        lwd = 1
      )

      segs <- GRanges(
        seqnames = object@seg$summary[[i]]$chrom,
        IRanges(
          start = object@seg$summary[[i]]$loc.start,
          end = object@seg$summary[[i]]$loc.end
        ),
        seqinfo = Seqinfo(genome = "hg19")
      )
      segs$seg.median <- object@seg$summary[[i]]$seg.median -
        object@bin$shift[i]
      segs <- segs[subjectHits(findOverlaps(
        query = detail.gene,
        subject = segs,
        type = "any"
      ))]
      lines(
        as.vector(rbind(rep(start(segs), each = 2), rep(end(segs), each = 2))),
        as.vector(rbind(NA, segs$seg.median, segs$seg.median, NA)),
        col = "darkblue",
        lwd = 2
      )
...
setMethod(
  ...
  function(
    ...
    if (set_par) {
      ...
      oma_original <- par()$oma
      par(
        mfrow = c(1, length(object@anno@detail) + 2),
        mar = c(8, 0, 4, 0),
        oma = c(0, 0, 4, 0)
      )
    }
      ...
      for (l in seq(length(object@anno@detail))) {
        if (l == 1) {
          CNV.detailplot(
            object,
            columns = i,
            name = object@anno@detail$name[l],
...
setMethod(
  ...
  function(
    ...
    segments.i <- GRanges(
      seqnames = y$Chromosome,
      ranges = IRanges(y$Start_Position, y$End_Position)
    )
    ...
    d_segments$balanced <- overview$count_balanced /
      length(unique(y$Sample)) *
      100
    ...
    segments_pl$seqnames <- factor(
      segments_pl$seqnames,
      levels = object@anno@genome$chr
    )
    ...
    abline(
      v = .cumsum0(object@anno@genome$size) + object@anno@genome$pq,
      col = "grey",
      lty = 2
    )
    axis(
    ...
    polygon(
      as.numeric(chr.cumsum0[match(segments_pl$seqnames, names(chr.cumsum0))]) +
        segments_pl$xpos,
      segments_pl$gains,
      col = "#F16729",
      lwd = 1.5
...
setMethod(
  ...
  function(
    ...
    b <- which(
      head(annotation$Chromosome, -1) != tail(annotation$Chromosome, -1)
    ) # between chromosomes
...
setMethod(
  "CNV.write",
  signature(object = "CNV.analysis"),
  function(object, file = NULL, what = "segments", threshold = 0.1) {
    w <- pmatch(
      what,
      c("probes", "bins", "detail", "segments", "gistic", "threshold", "focal")
    )
    if (w == 1) {
      if (length(object@fit) == 0) stop("fit unavailable, run CNV.fit")
      if (!is.null(file))
        if (!grepl(".igv$", file)) warning("filename does not end in .igv")

      x <- data.frame(
        Chromosome = as.vector(seqnames(object@anno@probes)),
      ...
      x <- data.frame(
        Chromosome = as.vector(seqnames(object@anno@bins)),
      ...
      if (length(object@detail) == 0) stop("detail unavailable, run CNV.bin")
      if (!is.null(file))
        if (!grepl(".txt$", file)) warning("filename does not end in .txt")

      x <- vector(mode = "list", length = ncol(object@fit$ratio))

      for (i in 1:ncol(object@fit$ratio)) {
        x[[i]] <- data.frame(
          Chromosome = as.vector(seqnames(object@anno@detail)),
          ...
          Value = round(object@detail$ratio[[i]] - object@bin$shift[i], 3),
      ...
      if (length(object@seg) == 0) stop("seg unavailable, run CNV.bin")
      if (!is.null(file))
        if (!grepl(".seg$", file)) warning("filename does not end in .seg")

      x <- vector(mode = "list", length = ncol(object@fit$ratio))

      for (i in 1:ncol(object@fit$ratio)) {
        x[[i]] <- data.frame(
          ID = colnames(object@fit$ratio)[i],
          ...
          seg.mean = round(
            object@seg$summary[[i]]$seg.mean -
              object@bin$shift[i],
            3
          ),
          seg.median = round(
            object@seg$summary[[i]]$seg.median -
              object@bin$shift[i],
            3
          ),
          row.names = NULL
      ...
      if (nrow(object@anno@genome) == 19) {
        warning(
          "GISTIC is not compatible with Illumina Methylation arrays for mice."
        )
      }
      ...
      for (i in 1:ncol(object@fit$ratio)) {
        ...
        y$seg.median <- round(
          object@seg$summary[[i]]$seg.median - object@bin$shift[i],
          3
        )
